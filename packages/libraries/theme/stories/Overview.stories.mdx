...
Real System was built with the purpose to easily theme and extend UI components for responsive web applications.
...
const MyComponent = () => {
  ...
  const bg = getToken('color-background');
  const space = getToken(5, 'space');
  const styles = {
    backgroundColor: bg,
    margin: space,
  };
