import { _logger } from '@real-system/utils';
...
const makeScaleUtil = (
  ...
  if (!Number.isInteger(size)) {
    _logger.throw.type(
      `utils/${origin}`,
      `majorScale only accepts integers as input, instead ${size} was passed.`
    );
  }
...
const organScale = (
  size: number,
  { format = 'rem', destructive = false }: ScalerOptions = defaultScalerOptions
) => makeScaleUtil(size, 2, { origin: 'traceScale', format, destructive });
/**
