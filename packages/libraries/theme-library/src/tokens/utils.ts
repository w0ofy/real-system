import { polished, StyleProps } from '@real-system/styling-library';
import { _get } from '@real-system/utils-library';

import type { Palette, PaletteKeys } from '../palettes/types';

import type { TokenPrefixes, TokenSuffixes } from './utils.types';
...
> = Record<`${T}-${O}-${TokenSuffixes}` | `${T}-${O}`, StyleProps['color']>;
...
const applyValueModifications =
  ...
  (val: string) => {
    ...
    if (override && typeof override === 'function') return override(val);
...
export { getPaletteColor, makeColorRange, makeEachColorRange };
